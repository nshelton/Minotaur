...
bool PlotSpooler::buildQueue(const PageModel &page, bool liftPen) {
    ...
    for (const auto &kv : page.entities) {
        ...
        if (e.type() != EntityType::PathSet) continue;
...
void PlotSpooler::run() {
    ...
    while (!m_cancel.load()) {
        ...
        if (m_paused.load()) {
            ...
            m_cv.wait(lk, [&]() { return !m_paused.load() || m_cancel.load(); });
        ...
        // Naive throttle: wait a fraction of last duration for SM, small delay otherwise
        if (cmd.kind == CmdKind::StepperMove) {
            const int sleepMs = std::max(1, cmd.durationMs - cmd.durationMs / 10);
